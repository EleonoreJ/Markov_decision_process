...
def loadfactor_monthly():
    ...
    for i_month in range(std_vals.shape[0]):
        ...
        #plt.plot(year_val,data_pt)
        #plt.plot(year_val,params[0]*year_val+params[1]);
#        plt.show()
        if i_month==0:
...
#s=np.random.normal(2019*bestfit_params[0,0]+bestfit_params[0,1],std_vals[0],10000)
...
# Extract data for the fuel function 
